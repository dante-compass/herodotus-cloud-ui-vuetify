...
本系统前后端数据加密传输，是基于自主设计的自定义数据传输 Session，实现国密 SM4 加密秘钥的动态生成，加密传输，一人一钥机制。每次使用系统都会动态重新生成，采用 SM2 前后端加密传输，临时存储至前端。缓存时间与 `OAuth 2 Token` 设置时间相同，正常退出系统后，会清除前端临时存储数据。详情参见：[【数字信封】](https://www.herodotus.vip/user-guide/security/security-extend.md#二-数字信封)
