...
var LayoutModeEnum$r = /* @__PURE__ */ ((LayoutModeEnum2) => {
...
let OptionsUtilities$e = class OptionsUtilities {
  // 静态私有实例引用
...
let RouterUtilities$e = class RouterUtilities {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
class SignOutUtilities {
  ...
  signOut(isLocal = false) {
    ...
    useCryptoStore$r().$reset();
    RouterUtilities$e.getInstance().toSignIn();
  }
  signOutWithDialog() {
    Swal.fire({
      title: "要走了么?",
...
let OAuth2ApiService$e = class OAuth2ApiService {
  // 静态私有实例引用
...
let OpenApiService$e = class OpenApiService {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
  ...
  verifyCaptcha(identity, category, data) {
    ...
    if (category === CaptchaCategoryEnum$r.WORD_CLICK) {
      verify.coordinates = data;
    } else if (category === CaptchaCategoryEnum$r.JIGSAW) {
...
let PasskeyApiService$e = class PasskeyApiService extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$e = class SecurityApiResources {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
  ...
  open() {
    return OpenApiService$e.getInstance(this.config);
  }
  oauth2() {
    return OAuth2ApiService$e.getInstance(this.config);
  }
  passkey() {
    return PasskeyApiService$e.getInstance(this.config);
  }
};
const useCryptoStore$r = defineStore("Crypto", {
  state: () => ({
    sessionId: "",
    key: "",
    state: ""
  ...
  actions: {
    ...
    setKey(key) {
      this.key = SM4Utils.encrypt(key, OptionsUtilities$e.getSecurityKey());
    },
    getKey() {
      return SM4Utils.decrypt(this.key, OptionsUtilities$e.getSecurityKey());
    },
    encrypt(content) {
    ...
    exchange(identity = "") {
      return new Promise((resolve, reject) => {
        SecurityApiResources$e.getInstance().open().createSession(identity).then((response) => {
          ...
          if (data) {
            ...
            SecurityApiResources$e.getInstance().open().exchange(sessionId, encryptData).then((response2) => {
...
const useAuthenticationStore$1 = defineStore("Authentication", {
  ...
  getters: {
    ...
    token() {
      if (OptionsUtilities$e.isAutoRefreshToken()) {
  ...
  actions: {
    ...
    setErrorPrompt(error, principal) {
      if (this.isAlertMessage(error)) {
        SecurityApiResources$e.getInstance().open().getPrompt(principal).then((result) => {
    ...
    signIn(username, password) {
      const crypto = useCryptoStore$r();
      if (OptionsUtilities$e.isUseCrypto()) {
        username = crypto.encrypt(username);
        password = crypto.encrypt(password);
      }
      return new Promise((resolve, reject) => {
        SecurityApiResources$e.getInstance().oauth2().passwordFlow(username, password, OptionsUtilities$e.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
    ...
    refreshToken() {
      return new Promise((resolve, reject) => {
        SecurityApiResources$e.getInstance().oauth2().refreshTokenFlow(this.refresh_token, OptionsUtilities$e.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.saveAccessToken(data);
          }
    ...
    signOut() {
      if (this.access_token) {
        SecurityApiResources$e.getInstance().oauth2().signOut(this.access_token).then(() => {
    ...
    authorizationCode(code, state = "") {
      return new Promise((resolve, reject) => {
        SecurityApiResources$e.getInstance().oauth2().authorizationCodeFlow(
          code,
          OptionsUtilities$e.getRedirectUri(),
          state,
          OptionsUtilities$e.isUseCrypto()
        ).then((response) => {
          if (response) {
            const data = response;
            this.saveAccessToken(data);
    ...
    smsSignIn(mobile, code) {
      ...
      return new Promise((resolve, reject) => {
        SecurityApiResources$e.getInstance().oauth2().socialCredentialsFlowBySms(mobile, code, OptionsUtilities$e.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.saveAccessToken(data);
          }
    ...
    socialSignIn(source, accessPrincipal) {
      return new Promise((resolve, reject) => {
        SecurityApiResources$e.getInstance().oauth2().socialCredentialsFlowByJustAuth(source, accessPrincipal, OptionsUtilities$e.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.saveAccessToken(data);
          }
    ...
    passkey(publicKey) {
      return new Promise((resolve, reject) => {
        SecurityApiResources$e.getInstance().oauth2().webAuthnCredentialsFlow(publicKey, OptionsUtilities$e.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.saveAccessToken(data);
          }
...
const getSystemHeaders = () => {
  ...
  const tenantId = OptionsUtilities$e.getTenantId();
  if (tenantId) {
    headers["X-Herodotus-Tenant-Id"] = tenantId;
  }
  return headers;
};
const useRouterStore$r = defineStore("Router", {
  state: () => ({
    routes: [],
    cachedRoutes: [],
...
const useSettingsStore = defineStore("GlobalSettings", {
  state: () => ({
     ...
     * 布局切换
     */
    layout: LayoutModeEnum$r.DEFAULT,
    /**
     * 界面效果
     */
    effect: {
  ...
  getters: {
    isDark: (state) => state.theme.mode === ThemeModeEnum$r.DARK,
    isLight: (state) => state.theme.mode === ThemeModeEnum$r.LIGHT,
    isSystem: (state) => state.theme.mode === ThemeModeEnum$r.SYSTEM,
    isDefaultLayout: (state) => state.layout === LayoutModeEnum$r.DEFAULT,
    isClassicLayout: (state) => state.layout === LayoutModeEnum$r.CLASSIC
  }
});
const useTabsViewStore = defineStore("TabsView", {
  ...
  actions: {
    ...
    isNotExistInStaticRoute(tab) {
      return lodash.findIndex(OptionsUtilities$e.getRoutes(), (item) => item.path === tab.path) === -1;
    },
    isTabNotOpened(tab) {
    ...
    smartTab(route) {
      ...
      if (isDetailRoute) {
        if (store.hasParameter(route)) {
          ...
          this.closeTab(tab);
          RouterUtilities$e.getInstance().goBack();
        }
      } else {
        this.openTab(tab, isDetailRoute);
      }
...
function useEditFinish() {
  ...
  const routeStore = useRouterStore$r();
...
function useDeviceAuthorize(clientId, clientSecret, deviceCode) {
  ...
  const process = () => {
    SecurityApiResources$e.getInstance().oauth2().deviceCodeFlow(clientId.value, clientSecret.value, deviceCode.value).then((response) => {
...
let OptionsUtilities$d = class OptionsUtilities22 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$d = class RouterUtilities22 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$d = class OAuth2ApiService22 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$d = class OpenApiService22 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$d = class PasskeyApiService22 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$d = class SecurityApiResources22 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OptionsUtilities$c = class OptionsUtilities2222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$c = class RouterUtilities2222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$c = class OAuth2ApiService2222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$c = class OpenApiService2222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$c = class PasskeyApiService2222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$c = class SecurityApiResources2222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OptionsUtilities$b = class OptionsUtilities222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$b = class RouterUtilities222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$b = class OAuth2ApiService222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$b = class OpenApiService222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$b = class PasskeyApiService222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$b = class SecurityApiResources222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OptionsUtilities$a = class OptionsUtilities22222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$a = class RouterUtilities22222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$a = class OAuth2ApiService22222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$a = class OpenApiService22222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$a = class PasskeyApiService22222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$a = class SecurityApiResources22222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OptionsUtilities$9 = class OptionsUtilities2222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$9 = class RouterUtilities2222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$9 = class OAuth2ApiService2222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$9 = class OpenApiService2222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$9 = class PasskeyApiService2222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$9 = class SecurityApiResources2222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OptionsUtilities$8 = class OptionsUtilities222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$8 = class RouterUtilities222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$8 = class OAuth2ApiService222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$8 = class OpenApiService222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$8 = class PasskeyApiService222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$8 = class SecurityApiResources222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OptionsUtilities$7 = class OptionsUtilities22222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$7 = class RouterUtilities22222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$7 = class OAuth2ApiService22222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$7 = class OpenApiService22222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$7 = class PasskeyApiService22222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$7 = class SecurityApiResources22222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OptionsUtilities$6 = class OptionsUtilities2222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$6 = class RouterUtilities2222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$6 = class OAuth2ApiService2222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$6 = class OpenApiService2222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$6 = class PasskeyApiService2222222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$6 = class SecurityApiResources2222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
const useCryptoStore$a = defineStore("Crypto", {
  ...
  actions: {
    ...
    setKey(key) {
      this.key = SM4Utils.encrypt(key, OptionsUtilities22222222222222222.getSecurityKey());
    },
    getKey() {
      return SM4Utils.decrypt(this.key, OptionsUtilities22222222222222222.getSecurityKey());
    },
    encrypt(content) {
      const key = this.getKey();
      return SM4Utils.encrypt(content, key);
...
defineStore("TabsView", {
  ...
  actions: {
    ...
    isNotExistInStaticRoute(tab) {
      return lodash.findIndex(OptionsUtilities22222222222222222.getRoutes(), (item) => item.path === tab.path) === -1;
    },
    isTabNotOpened(tab) {
      return this.getTabIndex(tab) === -1;
    },
...
let OptionsUtilities$5 = class OptionsUtilities222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$5 = class RouterUtilities222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$5 = class OAuth2ApiService222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$5 = class OpenApiService222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$5 = class PasskeyApiService222222222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$5 = class SecurityApiResources222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
const useCryptoStore$8 = defineStore("Crypto", {
  ...
  actions: {
    ...
    setKey(key) {
      this.key = SM4Utils.encrypt(key, OptionsUtilities2222222222222222222.getSecurityKey());
    },
    getKey() {
      return SM4Utils.decrypt(this.key, OptionsUtilities2222222222222222222.getSecurityKey());
    },
    encrypt(content) {
      const key = this.getKey();
      return SM4Utils.encrypt(content, key);
...
defineStore("TabsView", {
  ...
  actions: {
    ...
    isNotExistInStaticRoute(tab) {
      return lodash.findIndex(OptionsUtilities2222222222222222222.getRoutes(), (item) => item.path === tab.path) === -1;
    },
    isTabNotOpened(tab) {
      return this.getTabIndex(tab) === -1;
    },
...
let OptionsUtilities$4 = class OptionsUtilities22222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$4 = class RouterUtilities22222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$4 = class OAuth2ApiService22222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$4 = class OpenApiService22222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$4 = class PasskeyApiService22222222222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$4 = class SecurityApiResources22222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
const useCryptoStore$6 = defineStore("Crypto", {
  ...
  actions: {
    ...
    setKey(key) {
      this.key = SM4Utils.encrypt(key, OptionsUtilities222222222222222222222.getSecurityKey());
    },
    getKey() {
      return SM4Utils.decrypt(this.key, OptionsUtilities222222222222222222222.getSecurityKey());
    },
    encrypt(content) {
      const key = this.getKey();
      return SM4Utils.encrypt(content, key);
...
defineStore("TabsView", {
  ...
  actions: {
    ...
    isNotExistInStaticRoute(tab) {
      return lodash.findIndex(OptionsUtilities222222222222222222222.getRoutes(), (item) => item.path === tab.path) === -1;
    },
    isTabNotOpened(tab) {
      return this.getTabIndex(tab) === -1;
    },
...
let OptionsUtilities$3 = class OptionsUtilities2222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$3 = class RouterUtilities2222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$3 = class OAuth2ApiService2222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$3 = class OpenApiService2222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$3 = class PasskeyApiService2222222222222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$3 = class SecurityApiResources2222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
class OptionsUtilities22222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
class RouterUtilities22222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
class OAuth2ApiService22222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
class OpenApiService22222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
class PasskeyApiService22222222222222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
class SecurityApiResources22222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
  ...
  open() {
    return OpenApiService22222222222222222222222.getInstance(this.config);
  }
  oauth2() {
    return OAuth2ApiService22222222222222222222222.getInstance(this.config);
  }
  passkey() {
    return PasskeyApiService22222222222222222222222.getInstance(this.config);
  }
}
const useCryptoStore$4 = defineStore("Crypto", {
  ...
  actions: {
    ...
    setKey(key) {
      this.key = SM4Utils.encrypt(key, OptionsUtilities22222222222222222222222.securityKey());
    },
    getKey() {
      return SM4Utils.decrypt(this.key, OptionsUtilities22222222222222222222222.securityKey());
    },
    encrypt(content) {
      const key = this.getKey();
      return SM4Utils.encrypt(content, key);
    ...
    exchange(identity = "") {
      return new Promise((resolve, reject) => {
        SecurityApiResources22222222222222222222222.getInstance().open().createSession(identity).then((response) => {
          const data = response.data;
          if (data) {
            const sessionId = data.sessionId;
            const backendPublicKey = data.publicKey;
            ...
            const pair = SM2Utils.createKeyPair();
            const encryptData = SM2Utils.encrypt(pair.publicKey, backendPublicKey);
            SecurityApiResources22222222222222222222222.getInstance().open().exchange(sessionId, encryptData).then((response2) => {
              const confidential = response2.data;
              const key = SM2Utils.decrypt(confidential, pair.privateKey);
              this.setSessionId(sessionId);
              this.setKey(key);
...
defineStore("Authentication", {
  ...
  getters: {
    ...
    token() {
      if (OptionsUtilities22222222222222222222222.isAutoRefreshToken()) {
        return this.access_token;
      } else {
        if (this.isNotExpired) {
          return this.access_token;
  ...
  actions: {
    ...
    setErrorPrompt(error, principal) {
      if (this.isAlertMessage(error)) {
        SecurityApiResources22222222222222222222222.getInstance().open().getPrompt(principal).then((result) => {
          this.setUserErrorStatus(result.data);
        });
      }
    },
    signIn(username, password) {
      const crypto = useCryptoStore$4();
      if (OptionsUtilities22222222222222222222222.isUseCrypto()) {
        username = crypto.encrypt(username);
        password = crypto.encrypt(password);
      }
      return new Promise((resolve, reject) => {
        SecurityApiResources22222222222222222222222.getInstance().oauth2().passwordFlow(username, password, OptionsUtilities22222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    refreshToken() {
      return new Promise((resolve, reject) => {
        SecurityApiResources22222222222222222222222.getInstance().oauth2().refreshTokenFlow(this.refresh_token, OptionsUtilities22222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    signOut() {
      if (this.access_token) {
        SecurityApiResources22222222222222222222222.getInstance().oauth2().signOut(this.access_token).then(() => {
          console.log("Server side sign out successfully.");
        }).catch((error) => {
          console.log("Server side sign out has error.", error);
        });
    ...
    authorizationCode(code, state = "") {
      return new Promise((resolve, reject) => {
        SecurityApiResources22222222222222222222222.getInstance().oauth2().authorizationCodeFlow(
          code,
          OptionsUtilities22222222222222222222222.getRedirectUri(),
          state,
          OptionsUtilities22222222222222222222222.isUseCrypto()
        ).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
    ...
    smsSignIn(mobile, code) {
      ...
      return new Promise((resolve, reject) => {
        SecurityApiResources22222222222222222222222.getInstance().oauth2().socialCredentialsFlowBySms(mobile, code, OptionsUtilities22222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    socialSignIn(source, accessPrincipal) {
      return new Promise((resolve, reject) => {
        SecurityApiResources22222222222222222222222.getInstance().oauth2().socialCredentialsFlowByJustAuth(source, accessPrincipal, OptionsUtilities22222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    passkey(publicKey) {
      return new Promise((resolve, reject) => {
        SecurityApiResources22222222222222222222222.getInstance().oauth2().webAuthnCredentialsFlow(publicKey, OptionsUtilities22222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
...
defineStore("TabsView", {
  ...
  actions: {
    ...
    isNotExistInStaticRoute(tab) {
      return lodash.findIndex(
        OptionsUtilities22222222222222222222222.getInstance().getOptions().staticRoutes,
        (item) => item.path === tab.path
      ) === -1;
    },
    isTabNotOpened(tab) {
    ...
    smartTab(route) {
      ...
      if (isDetailRoute) {
        if (store.hasParameter(route)) {
          ...
          this.closeTab(tab);
          RouterUtilities22222222222222222222222.getInstance().goBack();
        }
      } else {
        this.openTab(tab, isDetailRoute);
      }
...
let OptionsUtilities$2 = class OptionsUtilities222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$2 = class RouterUtilities222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$2 = class OAuth2ApiService222222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$2 = class OpenApiService222222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$2 = class PasskeyApiService222222222222222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$2 = class SecurityApiResources222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
class OptionsUtilities2222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
class RouterUtilities2222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
defineStore("TabsView", {
  ...
  actions: {
    ...
    isNotExistInStaticRoute(tab) {
      return lodash.findIndex(
        OptionsUtilities2222222222222222222222222.getInstance().getOptions().staticRoutes,
        (item) => item.path === tab.path
      ) === -1;
    },
    isTabNotOpened(tab) {
    ...
    smartTab(route) {
      ...
      if (isDetailRoute) {
        if (store.hasParameter(route)) {
          ...
          this.closeTab(tab);
          RouterUtilities2222222222222222222222222.getInstance().goBack();
        }
      } else {
        this.openTab(tab, isDetailRoute);
      }
...
class OAuth2ApiService2222222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
class OpenApiService2222222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
class PasskeyApiService2222222222222222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
class SecurityApiResources2222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
  ...
  open() {
    return OpenApiService2222222222222222222222222.getInstance(this.config);
  }
  oauth2() {
    return OAuth2ApiService2222222222222222222222222.getInstance(this.config);
  }
  passkey() {
    return PasskeyApiService2222222222222222222222222.getInstance(this.config);
  }
}
const useCryptoStore$2 = defineStore("Crypto", {
  ...
  actions: {
    ...
    setKey(key) {
      this.key = SM4Utils.encrypt(key, OptionsUtilities2222222222222222222222222.securityKey());
    },
    getKey() {
      return SM4Utils.decrypt(this.key, OptionsUtilities2222222222222222222222222.securityKey());
    },
    encrypt(content) {
      const key = this.getKey();
      return SM4Utils.encrypt(content, key);
    ...
    exchange(identity = "") {
      return new Promise((resolve, reject) => {
        SecurityApiResources2222222222222222222222222.getInstance().open().createSession(identity).then((response) => {
          const data = response.data;
          if (data) {
            const sessionId = data.sessionId;
            const backendPublicKey = data.publicKey;
            ...
            const pair = SM2Utils.createKeyPair();
            const encryptData = SM2Utils.encrypt(pair.publicKey, backendPublicKey);
            SecurityApiResources2222222222222222222222222.getInstance().open().exchange(sessionId, encryptData).then((response2) => {
              const confidential = response2.data;
              const key = SM2Utils.decrypt(confidential, pair.privateKey);
              this.setSessionId(sessionId);
              this.setKey(key);
...
defineStore("Authentication", {
  ...
  getters: {
    ...
    token() {
      if (OptionsUtilities2222222222222222222222222.isAutoRefreshToken()) {
        return this.access_token;
      } else {
        if (this.isNotExpired) {
          return this.access_token;
  ...
  actions: {
    ...
    setErrorPrompt(error, principal) {
      if (this.isAlertMessage(error)) {
        SecurityApiResources2222222222222222222222222.getInstance().open().getPrompt(principal).then((result) => {
          this.setUserErrorStatus(result.data);
        });
      }
    },
    signIn(username, password) {
      const crypto = useCryptoStore$2();
      if (OptionsUtilities2222222222222222222222222.isUseCrypto()) {
        username = crypto.encrypt(username);
        password = crypto.encrypt(password);
      }
      return new Promise((resolve, reject) => {
        SecurityApiResources2222222222222222222222222.getInstance().oauth2().passwordFlow(username, password, OptionsUtilities2222222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    refreshToken() {
      return new Promise((resolve, reject) => {
        SecurityApiResources2222222222222222222222222.getInstance().oauth2().refreshTokenFlow(this.refresh_token, OptionsUtilities2222222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    signOut() {
      if (this.access_token) {
        SecurityApiResources2222222222222222222222222.getInstance().oauth2().signOut(this.access_token).then(() => {
          console.log("Server side sign out successfully.");
        }).catch((error) => {
          console.log("Server side sign out has error.", error);
        });
    ...
    authorizationCode(code, state = "") {
      return new Promise((resolve, reject) => {
        SecurityApiResources2222222222222222222222222.getInstance().oauth2().authorizationCodeFlow(
          code,
          OptionsUtilities2222222222222222222222222.getRedirectUri(),
          state,
          OptionsUtilities2222222222222222222222222.isUseCrypto()
        ).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
    ...
    smsSignIn(mobile, code) {
      ...
      return new Promise((resolve, reject) => {
        SecurityApiResources2222222222222222222222222.getInstance().oauth2().socialCredentialsFlowBySms(mobile, code, OptionsUtilities2222222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    socialSignIn(source, accessPrincipal) {
      return new Promise((resolve, reject) => {
        SecurityApiResources2222222222222222222222222.getInstance().oauth2().socialCredentialsFlowByJustAuth(source, accessPrincipal, OptionsUtilities2222222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    passkey(publicKey) {
      return new Promise((resolve, reject) => {
        SecurityApiResources2222222222222222222222222.getInstance().oauth2().webAuthnCredentialsFlow(publicKey, OptionsUtilities2222222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
...
let OptionsUtilities$1 = class OptionsUtilities22222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let RouterUtilities$1 = class RouterUtilities22222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
let OAuth2ApiService$1 = class OAuth2ApiService22222222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let OpenApiService$1 = class OpenApiService22222222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
...
let PasskeyApiService$1 = class PasskeyApiService22222222222222222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
...
let SecurityApiResources$1 = class SecurityApiResources22222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
...
class OptionsUtilities222222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
   ...
   */
  static initialize(options) {
    if (OptionsUtilities222222222222222222222222222._initialized) {
      throw new Error("RouterUtilities has already been initialized");
    }
    OptionsUtilities222222222222222222222222222._instance = new OptionsUtilities222222222222222222222222222(options);
    OptionsUtilities222222222222222222222222222._initialized = true;
    return OptionsUtilities222222222222222222222222222._instance;
  }
  /**
   * 获取单例实例
   * @returns {RouterUtilities} 单例实例
   */
  static getInstance() {
    if (!OptionsUtilities222222222222222222222222222._instance) {
      throw new Error("RouterUtilities not initialized. Call initialize() first.");
    }
    return OptionsUtilities222222222222222222222222222._instance;
  }
...
class RouterUtilities222222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
   ...
   */
  static initialize(options) {
    if (RouterUtilities222222222222222222222222222._initialized) {
      throw new Error("RouterUtilities has already been initialized");
    }
    RouterUtilities222222222222222222222222222._instance = new RouterUtilities222222222222222222222222222(options);
    RouterUtilities222222222222222222222222222._initialized = true;
    return RouterUtilities222222222222222222222222222._instance;
  }
  /**
   * 获取单例实例
   * @returns {RouterUtilities} 单例实例
   */
  static getInstance() {
    if (!RouterUtilities222222222222222222222222222._instance) {
      throw new Error("RouterUtilities not initialized. Call initialize() first.");
    }
    return RouterUtilities222222222222222222222222222._instance;
  }
...
defineStore("TabsView", {
  ...
  actions: {
    ...
    isNotExistInStaticRoute(tab) {
      return lodash.findIndex(
        OptionsUtilities222222222222222222222222222.getInstance().getOptions().staticRoutes,
        (item) => item.path === tab.path
      ) === -1;
    },
    isTabNotOpened(tab) {
    ...
    smartTab(route) {
      ...
      if (isDetailRoute) {
        if (store.hasParameter(route)) {
          ...
          this.closeTab(tab);
          RouterUtilities222222222222222222222222222.getInstance().goBack();
        }
      } else {
        this.openTab(tab, isDetailRoute);
      }
...
class OAuth2ApiService222222222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
  ...
  static getInstance(config) {
    if (this.instance == null) {
      this.instance = new OAuth2ApiService222222222222222222222222222(config);
    }
    return this.instance;
  }
  getOAuth2TokenAddress() {
...
class OpenApiService222222222222222222222222222 {
  // 静态私有实例引用
  static instance = null;
  config = {};
  constructor(config) {
  ...
  static getInstance(config) {
    if (this.instance == null) {
      this.instance = new OpenApiService222222222222222222222222222(config);
    }
    return this.instance;
  }
  createSession(sessionId = "") {
...
class PasskeyApiService222222222222222222222222222 extends Service {
  // 静态私有实例引用
  static instance = null;
  constructor(config) {
    super(config);
  ...
  static getInstance(config) {
    if (this.instance == null) {
      this.instance = new PasskeyApiService222222222222222222222222222(config);
    }
    return this.instance;
  }
  getBaseAddress() {
...
class SecurityApiResources222222222222222222222222222 {
  // 静态私有实例引用
  static _instance = null;
  // 初始化标志
  static _initialized = false;
   ...
   */
  static initialize(config) {
    if (SecurityApiResources222222222222222222222222222._initialized) {
      throw new Error("SecurityApiResources has already been initialized");
    }
    SecurityApiResources222222222222222222222222222._instance = new SecurityApiResources222222222222222222222222222(config);
    SecurityApiResources222222222222222222222222222._initialized = true;
    return SecurityApiResources222222222222222222222222222._instance;
  }
  /**
   * 获取单例实例
   * @returns {RouterUtilities} 单例实例
   */
  static getInstance() {
    if (!SecurityApiResources222222222222222222222222222._instance) {
      throw new Error("SecurityApiResources not initialized. Call initialize() first.");
    }
    return SecurityApiResources222222222222222222222222222._instance;
  }
  ...
  open() {
    return OpenApiService222222222222222222222222222.getInstance(this.config);
  }
  oauth2() {
    return OAuth2ApiService222222222222222222222222222.getInstance(this.config);
  }
  passkey() {
    return PasskeyApiService222222222222222222222222222.getInstance(this.config);
  }
}
const useCryptoStore = defineStore("Crypto", {
  ...
  actions: {
    ...
    setKey(key) {
      this.key = SM4Utils.encrypt(key, OptionsUtilities222222222222222222222222222.securityKey());
    },
    getKey() {
      return SM4Utils.decrypt(this.key, OptionsUtilities222222222222222222222222222.securityKey());
    },
    encrypt(content) {
      const key = this.getKey();
      return SM4Utils.encrypt(content, key);
    ...
    exchange(identity = "") {
      return new Promise((resolve, reject) => {
        SecurityApiResources222222222222222222222222222.getInstance().open().createSession(identity).then((response) => {
          const data = response.data;
          if (data) {
            const sessionId = data.sessionId;
            const backendPublicKey = data.publicKey;
            ...
            const pair = SM2Utils.createKeyPair();
            const encryptData = SM2Utils.encrypt(pair.publicKey, backendPublicKey);
            SecurityApiResources222222222222222222222222222.getInstance().open().exchange(sessionId, encryptData).then((response2) => {
              const confidential = response2.data;
              const key = SM2Utils.decrypt(confidential, pair.privateKey);
              this.setSessionId(sessionId);
              this.setKey(key);
...
defineStore("Authentication", {
  ...
  getters: {
    ...
    token() {
      if (OptionsUtilities222222222222222222222222222.isAutoRefreshToken()) {
        return this.access_token;
      } else {
        if (this.isNotExpired) {
          return this.access_token;
  ...
  actions: {
    ...
    setErrorPrompt(error, principal) {
      if (this.isAlertMessage(error)) {
        SecurityApiResources222222222222222222222222222.getInstance().open().getPrompt(principal).then((result) => {
          this.setUserErrorStatus(result.data);
        });
      }
    },
    signIn(username, password) {
      const crypto = useCryptoStore();
      if (OptionsUtilities222222222222222222222222222.isUseCrypto()) {
        username = crypto.encrypt(username);
        password = crypto.encrypt(password);
      }
      return new Promise((resolve, reject) => {
        SecurityApiResources222222222222222222222222222.getInstance().oauth2().passwordFlow(username, password, OptionsUtilities222222222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    refreshToken() {
      return new Promise((resolve, reject) => {
        SecurityApiResources222222222222222222222222222.getInstance().oauth2().refreshTokenFlow(this.refresh_token, OptionsUtilities222222222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    signOut() {
      if (this.access_token) {
        SecurityApiResources222222222222222222222222222.getInstance().oauth2().signOut(this.access_token).then(() => {
          console.log("Server side sign out successfully.");
        }).catch((error) => {
          console.log("Server side sign out has error.", error);
        });
    ...
    authorizationCode(code, state = "") {
      return new Promise((resolve, reject) => {
        SecurityApiResources222222222222222222222222222.getInstance().oauth2().authorizationCodeFlow(
          code,
          OptionsUtilities222222222222222222222222222.getRedirectUri(),
          state,
          OptionsUtilities222222222222222222222222222.isUseCrypto()
        ).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
    ...
    smsSignIn(mobile, code) {
      ...
      return new Promise((resolve, reject) => {
        SecurityApiResources222222222222222222222222222.getInstance().oauth2().socialCredentialsFlowBySms(mobile, code, OptionsUtilities222222222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    socialSignIn(source, accessPrincipal) {
      return new Promise((resolve, reject) => {
        SecurityApiResources222222222222222222222222222.getInstance().oauth2().socialCredentialsFlowByJustAuth(source, accessPrincipal, OptionsUtilities222222222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
    ...
    passkey(publicKey) {
      return new Promise((resolve, reject) => {
        SecurityApiResources222222222222222222222222222.getInstance().oauth2().webAuthnCredentialsFlow(publicKey, OptionsUtilities222222222222222222222222222.isUseCrypto()).then((response) => {
          if (response) {
            const data = response;
            this.setTokenInfo(data);
          }
...
function useVuetifyMenu() {
  ...
  const getMenuItems = () => {
    const routers = RouterUtilities$e.getInstance().getRouter().getRoutes();
...
function usePasskey() {
  ...
  const registration = (label) => {
    return new Promise((resolve, reject) => {
      SecurityApiResources$e.getInstance().passkey().fetchWebAuthnRegisterOptions().then((publicKey) => {
        ...
        create(registrationOptions).then((registration2) => {
          ...
          SecurityApiResources$e.getInstance().passkey().webAuthnRegister(request).then(() => {
            resolve(true);
          });
        });
      }).catch(() => {
  ...
  const authenticator = () => {
    return new Promise((resolve, reject) => {
      SecurityApiResources$e.getInstance().passkey().fetchWebAuthnAuthenticateOptions().then((publicKey) => {
...
function useSystemTheme() {
  ...
  const systemTheme = shallowRef(ThemeModeEnum$r.DARK);
  ...
  const onThemeChange = () => {
    systemTheme.value = media.matches ? ThemeModeEnum$r.DARK : ThemeModeEnum$r.LIGHT;
  };
  watch(
    () => settings.theme.mode,
    (val) => {
      if (val === ThemeModeEnum$r.SYSTEM) {
        media = getMatchMedia();
...
const initializer = (options) => {
  OptionsUtilities$e.initialize(options);
  RouterUtilities$e.initialize(options.router);
  SecurityApiResources$e.initialize(options.config);
  SignOutUtilities.initialize(options.signOutExtension);
};
export {
  CaptchaCategoryEnum$r as CaptchaCategoryEnum,
  LayoutModeEnum$r as LayoutModeEnum,
  OptionsUtilities$e as OptionsUtilities,
  RouterUtilities$e as RouterUtilities,
  SecurityApiResources$e as SecurityApiResources,
  SignOutUtilities,
  SocialSourceEnum,
  ThemeModeEnum$r as ThemeModeEnum,
  getSystemHeaders,
  initializer,
  useApplicationStore,
  useAuthenticationStore$1 as useAuthenticationStore,
  useCryptoStore$r as useCryptoStore,
  useDeviceAuthorize,
  useEditFinish,
  useQuasarMenu as useMenuForQuasar,
  useVuetifyMenu as useMenuForVuetify,
  usePasskey,
  useRouterStore$r as useRouterStore,
  useSettingsStore,
  useSystemRoute,
  useSystemTheme,
  useTabsViewStore
