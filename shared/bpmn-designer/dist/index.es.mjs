...
import { add as add2 } from "diagram-js/lib/util/Collections";
...
function usePropertyElements() {
  ...
  const getFirstExtensionElementsValue = (extensionElements) => {
    ...
    return !lodash.isEmpty(values) ? values[0] : {};
  };
  const setExtensionElementsValues = (parentElement, extensionElements, values) => {
...
function useConditionProperties() {
  ...
  const fetchConditionExpression = (element, create = false) => {
    const businessObject = getBusinessObject(element);
...
function useExtensionProperties() {
  ...
  const getProperties = (extensionElements) => {
    const values = getExtensionElementsValues(extensionElements);
    return !lodash.isEmpty(values) ? values[0] : {};
  };
...
function useGeneralProperties() {
  ...
  const isTextAnnotation = (element) => {
    return is(element, "bpmn:TextAnnotation");
  };
  const isGroup = (element) => {
    return is(element, "bpmn:Group");
  };
  const createCategoryValue = (definitions) => {
...
const getBpmnIcon = (index2) => {
  const name = ImageIndexes[index2];
  return name;
};
...
const parseElementType = (element) => {
  ...
  if (eventDefinition) {
    ...
    if (is(element, "bpmn:StartEvent") && !isInterrupting(element) || is(element, "bpmn:BoundaryEvent") && businessObject.cancelActivity === false) {
...
function useModelerCreator() {
  ...
  const createEmptyDiagram = (key, name) => {
    ...
    const fetchAllItems = async () => {
      ...
      if (!lodash.isEmpty(data)) {
        options.value = data.map((i) => {
          return { label: i.name + "-" + i.activityName, value: i.id };
        });
      }
    };
    onMounted(() => {
    ...
    const downloadProcess = async (type, name = "diagram") => {
      try {
        if (type === "xml" || type === "bpmn") {
          const { xml: xml2 } = await designer.modeler.saveXML();
          const { href, filename } = downloadEncode(type, name, xml2);
...
export {
  ...
  getAssetsFile,
  getBpmnIcon,
  getBusinessObject2 as getBusinessObject,
  getRawType,
  is2 as is,
  isAny2 as isAny,
  lodash2 as lodash,
