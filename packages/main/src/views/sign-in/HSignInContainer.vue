      ...
      <v-col align-self="center">
        <v-hover v-slot="{ props: hoverProps, isHovering }">
          <v-card
            style="z-index: 5"
            v-bind="hoverProps"
            :class="['elevation-20 mx-auto rounded-xl', isHovering ? 'opacity-100' : 'opacity-70']"
          >
            <template #append>
...
defineOptions({ name: 'HSignInContainer', components: { HAppLogo } });

const { onCycleChangeTheme, cycleChangeThemeIcon } = useSystemTheme();
